# superclaw doctor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `superclaw doctor [--fix] [--verbose]` command that checks 27 items across 5 categories (prerequisites, installation, configuration, runtime, connectivity) and can auto-fix common issues.

**Architecture:** Doctor logic lives in a new `cli/doctor.mjs` module, imported by `superclaw.mjs` for dispatch. Each check category is a function returning an array of `{name, status, message, fix?}` results. The `--fix` flag triggers fix functions on fixable items. Reuses existing utilities from `superclaw.mjs` via import.

**Tech Stack:** Node.js ESM, child_process (spawnSync/execSync), fs

**Spec:** `docs/superpowers/specs/2026-04-08-doctor-design.md`
...
|------|--------|---------------|
| `cli/doctor.mjs` | Create | All 27 checks organized in 5 category functions + fix logic + output formatting |
| `cli/superclaw.mjs` | Modify | Add `doctor` command to parseArgs, dispatch, help text, isShortSubcommand |
| `cli/SKILL.md` | Modify | Document doctor command |

---

### Task 1: Create `cli/doctor.mjs` with check framework and output formatting

**Files:**
- Create: `cli/doctor.mjs`

- [ ] **Step 1: Create `cli/doctor.mjs` with core framework**

Create `cli/doctor.mjs` with the result types, runner, and output formatter:
  ...
  for (const [catName, results] of categories) {
    ...
    if (verbose) {
      for (const r of results) {
        const sym = r.status === "pass" ? "✅" : r.status === "warn" ? "⚠️ " : "❌";
    ...
    for (const r of results) {
      if (r.status !== "pass" && r.fix) {
        try {
          ...
          console.log(`  ❌ Fix failed for ${r.name}: ${e.message}`);
        }
      }
    }
  }
...
```bash
git add cli/doctor.mjs
git commit -m "feat(doctor): add check framework and output formatter"
```

---

### Task 2: Implement all 5 check categories in `cli/doctor.mjs`

**Files:**
- Modify: `cli/doctor.mjs`

- [ ] **Step 1: Add Prerequisites checks (7 items)**

Append to `cli/doctor.mjs`:
...
- [ ] **Step 2: Add Installation checks (9 items)**
...
function checkSymlink(path, label, expectedTarget) {
  try {
    ...
    if (stat.isSymbolicLink()) {
      const target = readFileSync(path, "utf8"); // will follow symlink for content; use readlinkSync
...
function checkInstallation(ctx) {
  ...
  if (ctx.installed) {
    try {
      const head = execSync("git rev-parse HEAD", { cwd: repoDir, encoding: "utf8", timeout: 3000 }).trim();
...
- [ ] **Step 3: Add Configuration checks (4 items)**
...
- [ ] **Step 4: Add Runtime checks (4 items)**
...
- [ ] **Step 5: Add Connectivity checks (3 items)**
...
- [ ] **Step 6: Add the exported `runDoctor` entry point**
...
- [ ] **Step 7: Test the module standalone**

```bash
cd /root/.openclaw/workspace/repos/superclaw
node -e "import('./cli/doctor.mjs').then(m => m.runDoctor({ fix: false, verbose: true, binDir: '/root/.openclaw/workspace/bin', repoDir: '/root/.openclaw/workspace/repos/superclaw', installedJsonPath: '/root/.superclaw/installed.json' }))"
```

Expected: All 5 categories printed with check results.

- [ ] **Step 8: Commit**

```bash
git add cli/doctor.mjs
git commit -m "feat(doctor): implement 27 health checks across 5 categories

Categories: Prerequisites (7), Installation (9), Configuration (4),
Runtime (4), Connectivity (3). Supports --fix for auto-repair and
--verbose for full output."
```

---

### Task 3: Wire `doctor` into `cli/superclaw.mjs`
...
At the top of `cli/superclaw.mjs`, after the existing imports (after line 16 `import process from "node:process";`), add:

```js
import { runDoctor } from "./doctor.mjs";
```

- [ ] **Step 2: Add to parseArgs**

In `parseArgs`, after the `update` command recognition block, add:
...
Add to flag parsing (near the `--check` / `--signal` / `--dry-run` section):
...
- [ ] **Step 3: Add to printUsage**

Add after the `superclaw update [--check]` line:

```js
    "  superclaw doctor [--fix] [--verbose]",
```

- [ ] **Step 4: Add to dispatch in main()**

In the `switch (opts.command)` block, add before `default:`:

```js
    case "doctor": {
      // Doctor loads .env itself (best-effort), doesn't need ensureEnv to hard-fail
...
**Important:** Doctor must NOT go through `ensureEnv()` (which hard-fails on missing .env vars). Move the `ensureEnv()` call to AFTER the command parse, and skip it for `doctor` and `version`:
...
Also remove the old `if (!opts.command)` block that was after parseArgs.

- [ ] **Step 5: Add to isShortSubcommand**

Add `"doctor"` to the short subcommand list:
...
git commit -m "feat(cli): wire doctor command into superclaw dispatch

Also skip ensureEnv() for doctor and version commands so they can
run without .env configured (diagnostic use case)."
```
...
- [ ] **Step 1: Add doctor command to SKILL.md**

After the `### Update superclaw` section, add:
...
Checks 27 items across 5 categories: prerequisites, installation, configuration, runtime, connectivity. Use `--verbose` to see all checks (including passed). Use `--fix` to auto-repair fixable issues (stale sessions, broken symlinks, log rotation).
```
...
```bash
git add cli/SKILL.md
git commit -m "docs: add doctor command to CLI skill documentation"
```
...
- [ ] **Step 1: Run doctor default mode**

```bash
superclaw doctor
```

Expected: 5 categories with pass/fail/warn counts, exit 0 if no failures.

- [ ] **Step 2: Run doctor verbose**

```bash
superclaw doctor --verbose
```

Expected: All 27 checks listed individually with ✅/⚠️/❌.

- [ ] **Step 3: Run doctor with fix**

```bash
# Create a stale session to test fix
echo '{"session_name":"doctor-test","cwd":"/tmp","model":"opus","pid":999999,"start_time":"2025-01-01T00:00:00Z"}' > ~/.superclaw/state/sessions/doctor-test.json

superclaw doctor --fix
```

Expected: Stale session detected and cleaned.
...
Expected: doctor command listed.

- [ ] **Step 5: Verify exit codes**

```bash
superclaw doctor; echo "exit: $?"
```

Expected: exit 0 when no failures, exit 1 when failures present.
