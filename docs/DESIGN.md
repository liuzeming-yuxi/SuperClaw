...
- [x] plan 阶段 OpenClaw 和 Claude Code 的对话用什么形式？**已解决：通过 cc-delegate 支持 exec（一次性）和 session（持久会话）两种模式。**
- [ ] verify 阶段 E2E 测试用例怎么生成？从 spec 自动推导？还是手写？
- [x] cc-delegate 是否需要扩展新命令来支持 plan 阶段的交互式对话？**已解决：session start/continue 支持多轮对话。**
