...
│   ├── execute/SKILL.md         # Phase 3: 发起执行
│   ├── verify/SKILL.md          # Phase 4-L2: OpenClaw 验收
│   └── deliver/SKILL.md         # Phase 5: 人类验收交付
│
...
| 组件 | 说明 |
|---|---|
| `superclaw` | OpenClaw → Claude Code 桥接层 |
...
Phase 3 (execute):
  OpenClaw [execute skill]
    → superclaw 启动 Claude Code session
...
- [x] Claude Code 侧的 Superpowers skill 需要改造多少？**已解决：直接复用，不 fork 不改造。**
- [x] plan 阶段 OpenClaw 和 Claude Code 的对话用什么形式？**已解决：通过 superclaw 支持 exec（一次性）和 session（持久会话）两种模式。**
- [ ] verify 阶段 E2E 测试用例怎么生成？从 spec 自动推导？还是手写？
- [x] superclaw 是否需要扩展新命令来支持 plan 阶段的交互式对话？**已解决：session start/continue 支持多轮对话。**
- [x] Hook 的具体实现方式：用 Claude Code 原生 hook 还是 OpenClaw 侧触发？**已解决：使用 Claude Code 原生 hooks（Stop + PostToolUse），通过 shell 脚本桥接到 OpenClaw。**
